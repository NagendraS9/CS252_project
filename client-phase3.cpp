...
int main(int argc, char *argv[])
{
    ...
    // File buffers
    map<string, pair<ssize_t, char*>> filebuffers;
    for (int i=0;i<no_files;i++){
        filebuffers[file_names[i]] = {0, new char[BUFSIZ]};
    }

    string file_path = "./Downloaded/client";
    file_path += to_string(id);
    file_path += "/";
    int filefd;
    map<string, pair<bool, set<int>>> ffound;
    bool receiveFiles = false;
    ...
    for(;;) {
        ...
        //                 exit(EXIT_FAILURE);
        //             }
        //             filebuffers[it.first].first = read(mapfd[*(ffound.second.second.begin())], filebuffers[it.first].second, BUFSIZ);
        //             ssize_t read_return = filebuffers[it.first].first;
        //             if (read_return == -1) {
        //                 perror("read");
        //                 exit(EXIT_FAILURE);
        //             }
        //             if (write(filefd, filebuffers[it.first].second, read_return) == -1) {
        //                 perror("write");
        //                 exit(EXIT_FAILURE);
        //             }
        //             if (read_return <= 0){
