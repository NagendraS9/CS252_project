...
int main(int argc, char *argv[])
{
    ...
    map<string, pair<bool, set<int>>> ffound_copy;
    ...
    for (;;)
    {
        ...
        else if (!conDetails)
        {
            ...
            if (haveAllInfo)
            {
                ...
                // cout<<"559"<<askNeighbor<<receivedNN<<receivedAll<<endl;
        ...
        if (ask_for_files && !file_request_sended)
        {
            ...
            for (auto it : ffound_copy)
            {
                if (it.second.first)
                {
                    // cout<<"Found "<<it.first<<" at "<<*(ffound_copy[it.first].second.begin())<<" with MD5 0 at depth "<<file_depths[it.first]<<endl;
                    ...
                    else if (file_depths[it.first] == 2)
                    {
                        ...
                        {
                            if (it2.second.second[file_no].first == *(ffound_copy[it.first].second.begin()))
                            {
        ...
        if (!output_printed)
        {
            ...
            for (auto it : recieved)
            {
                if (!it.second)
                {
                    // cout<<"731 not reciev "<<it.first<<endl;
                    all_file_recieved = false;
                }
            }
            // cout<<all_file_recieved<<endl;
            if (all_file_recieved)
            {
                for (auto it : ffound)
                {
                    if (it.second.first)
                    {
                        ...
                        file_loc += "Downloaded/";
                        file_loc += it.first;
                        cout << "Found " << it.first << " at " << *(ffound[it.first].second.begin()) << " with MD5 " << find_md5(file_loc) << " at depth " << file_depths[it.first] << endl;
                    }
                    else
                    {
